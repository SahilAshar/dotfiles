---
name: code-review
description: 'Review changes since a fixed point (commit, branch, tag, or merge-base) along two axes — Standards (does the code follow this repo''s coding standards?) and Spec (does the code match what was asked for?). Runs both reviews in parallel sub-agents. Use when reviewing a branch, a PR, WIP changes, or "review since X".'
---
...
1. Issue references in the commit messages (`#123`, `Closes #45`, etc.) — fetch via `gh issue view` or `gh pr view`.
...
Anything in the repo that documents how code should be written: `CLAUDE.md`, `CODING_STANDARDS.md`, `CONTRIBUTING.md`, linter configs, etc.
...
Each smell reads *what it is* then *how to fix*:
