...
- **Prompt templates** — Re-invokes [`scripts/install-prompts.sh`](scripts/install-prompts.sh) so the `/generate` prompt is published to your VS Code user prompt directory. The helper always syncs from the canonical templates in [github/.github/prompts](github/.github/prompts) and logs the source and target paths.
...
Run `./install.sh` any time you change prompt templates. The installer republishes the `/generate` prompt by invoking the helper script for you and logs `Syncing Copilot prompts` followed by `Symlinking generate prompt` with the source and target paths so you can confirm which canonical directory it used.
...
For more details on how PR generation, custom prompts, and custom agents work, see [docs/pr-generation-workflow.md](docs/pr-generation-workflow.md).
